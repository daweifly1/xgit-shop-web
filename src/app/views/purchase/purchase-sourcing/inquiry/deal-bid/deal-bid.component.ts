...
import {SourcingInquiryService} from '../../../../../core/trans/purchase/sourcing-inquiry.service';
...
@Component({
  selector: 'app-deal-bid',
  templateUrl: './deal-bid.component.html',
  styleUrls: ['./deal-bid.component.scss']
})
export class DealBidComponent implements OnInit {

  @Input() inquiryId: string;
  @Input() apporvalId: string;
  ...
  constructor(private approvalFormService: ApprovalFormService, private messageService: ShowMessageService,
              private router: Router, private inquiryService: SourcingInquiryService, private utilService: UfastUtilService) {
  ...
  private getPriceSumInfo() {
    this.approvalFormService.getSupplierSumPrice(this.apporvalId).subscribe((resData) => {
  ...
  private getDetailInfo() {
    this.inquiryService.getDealBidDetail(this.apporvalId, this.inquiryId).subscribe((resData) => {
      this.getPriceSumInfo();
      this.purchaseInfo = resData.value;
      this.inquiryInfo = this.purchaseInfo.purchaseInquiryVO;
      this.inquiryInfo.inquiryNoNum = `${this.inquiryInfo.inquiryNo}-${this.inquiryInfo.count}`
      this.initTabData();
    });
  }
  private initTabData() {
  ...
  public createApprovalOk() {
    ...
    this.messageService.showAlertMessage('', '确定生成审定表？', 'confirm').afterClose
      .subscribe((type: string) => {
        ...
        this.approvalFormService.createApprovalOkForm(this.apporvalId, list).subscribe((resData) => {
          this.messageService.showToastMessage('操作成功', 'success');
